...
typedef struct {
    ...
    Jugador players[9];                // Lista de jugadores
    bool finished;                     // Si el juego se ha terminado
    char *board;                       // Puntero al inicio del tablero
} GameState;

typedef struct {
    sem_t canPrint;                   // Máster → Vista: hay cambios por imprimir
    sem_t completedPrint;             // Vista → Máster: terminó de imprimir
    // La sincronización entre el máster y la vista involucra los semáforos puedeImprimir y impresionCompleta siguiendo un patrón
    // simple de señalización bidireccional.
    ...
    // semáforos C, D y E y la variable F
